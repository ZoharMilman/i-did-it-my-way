...
def train_go1(headless=True, cfg_idx=0):
    ...
    num_of_iterations = 60000 # 35000 # was 100,000
...
#    logpath = Path(f"{MINI_GYM_ROOT_DIR}/runs").resolve() + "/" + logger.utcnow(f'gait-conditioned-agility/%Y-%m-%d/{stem}/%H%M%S.%f')
    
    mini_signature = logger.utcnow('%Y-%m-%d/%H%M%S.%f')
    date, time = mini_signature.split('/')
    signature = f'gait-conditioned-agility/{date}/{stem}/{time}'
    logger.configure(signature,
    ...
    logger.log_text(f"datetime: {mini_signature}", filename="enforce_summary.txt")
